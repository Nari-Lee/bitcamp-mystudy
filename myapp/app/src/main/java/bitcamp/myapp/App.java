...
public class App {
  ...
  private void loadData() {
    ...
    try (XSSFWorkbook workbook = new XSSFWorkbook("data.xlsx")) {
      ...
      System.out.println("데이터 로드 중 문제 발생");
      e.printStackTrace();
    }
  }
  ...
  private void loadUsers(XSSFWorkbook workbook) {
    ...
    try {
      ...
      System.out.println("회원 일련번호 초기화 오류");
      e.printStackTrace();
    }
  }
  ...
  private void loadBoards(XSSFWorkbook workbook) {
    ...
    for (int i = 1; i <= sheet.getLastRowNum(); i++) {
      ...
      try {
        ...
        System.out.printf("%s 번 게시판의 데이터 형식이 맞지 않습니다.\n", row.getCell(0).getStringCellValue());
      }
    }
    try {
      initSeqNo(projectList, Project.class);
    } catch (Exception e) {
      System.out.println("게시판 일련번호 초기화 오류");
      e.printStackTrace();
    }
  }
  ...
  private void loadProjects(XSSFWorkbook workbook) {
    ...
    try {
      ...
      System.out.println("프로젝트 일련번호 초기화 오류");
      e.printStackTrace();
    }
  }
  ...
  private void saveBoards(XSSFWorkbook workbook) {
    ...
    for (int i = 0; i < boardList.size(); i++) {
      ...
      dataRow.createCell(4).setCellValue(String.valueOf(board.getNo()));
    }
  }
